...
function deleteProvider(obj) {
    $.ajax({
        type: "GET",
        url: path + "/provider/delProvider.json",
...
$(function () {
    /*$(".viewProvider").on("click",function(){
        ...
        var obj = $(this);
        window.location.href=path+"/provider/providerView/"+ obj.attr("proid");
    });*/

    $(".viewProvider").on("click", function () {
        //将被绑定的元素（a）转换成jquery对象，可以使用jquery方法
        var obj = $(this);
        $.getJSON(path + "/provider/providerView.json", {id: obj.attr("proid")},
            function (data) {
                if (data != "null") {
    ...
    $(".modifyProvider").on("click", function () {
        var obj = $(this);
        window.location.href = path + "/provider/toProviderModify.html?proid=" + obj.attr("proid");
    });
