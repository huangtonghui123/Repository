...
$(function () {
    ...
    $(".viewRole").on("click", function () {
        //将被绑定的元素（a）转换成jquery对象，可以使用jquery方法
        var obj = $(this);
        $.getJSON(path + "/role/roleView.json", {roleid: obj.attr("roleid")},
            function (data) {
                if (data != "null") {
    ...
    $(".modifyRole").on("click", function () {
        var obj = $(this);
        window.location.href = path + "/role/toRoleModify.html?roleid=" + obj.attr("roleid");
    });
    ...
    $(".deleteRole").on("click", function () {
        roleObj = $(this);
        changeDLGContent("你确定要删除用户【" + roleObj.attr("username") + "】吗？");
        openYesOrNoDLG();
    });
